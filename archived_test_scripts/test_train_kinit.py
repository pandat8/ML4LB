...
def train(model, data_loader, optimizer=None):
    ...
    with torch.set_grad_enabled(optimizer is not None):
        for batch in data_loader:
            ...
            k_init = batch.k_init
            loss = F.mse_loss(k_model.float(), k_init.float())
...
def loaddata(instancetype, mode, ):
    directory_samples = './result/generated_instances/'+ instancetype +'/'+mode+'/' + 'samples_kinit/'
...
train_loader, valid_loader = loaddata(instancetype, mode)
train_loaders[instancetype] = train_loader
val_loaders[instancetype] = valid_loader

# instancetype = instancetypes[6]
# # train_loader, valid_loader = loaddata(instancetype, mode)
# directory_samples = './result/generated_instances/'+ instancetype +'/'+mode+'/' + 'samples_kinit/'
# filename = 'sample-kinit-'+ instancetype +'-*.pkl'
# print(filename)
...
train_instancetype = instancetypes[0]
valid_instancetype = instancetypes[0]
LEARNING_RATE = 0.00000005 # setcovering:0.0000005 cap-loc: 0.00000005 independentset: 0.0000001
EPOCHS = 20
